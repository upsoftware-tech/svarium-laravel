...
use Upsoftware\Svarium\Events\DeviceCreated;
use Upsoftware\Svarium\Events\DeviceHijacked;
use Upsoftware\Svarium\Events\DeviceUpdated;
use Upsoftware\Svarium\Events\UserSeenFromNewDevice;
use Upsoftware\Svarium\Events\UserSeenFromRogueDevice;
use Upsoftware\Svarium\Events\UserSeenFromUnverifiedDevice;
use Upsoftware\Svarium\Models\Device;
use Upsoftware\Svarium\Models\DeviceUser;
...
class DeviceTracking
{
    ...
    public function detect()
    {
        if (!isset($this->detectData)) {
            ...
            if (config('upsoftware.tracking.geoip_provider')) {

                $ip = Request::ip();
                /** @var GeoIpProvider */
                $geoipProvider = App::make(config('upsoftware.tracking.geoip_provider'));
    ...
    public function checkSessionDeviceHash()
    {
        if (Auth::guard('web')->check()) {
            ...
            $sessionData = session(config('upsoftware.tracking.session_key'));
    ...
    public function setSessionDeviceHash()
    {
        if (Auth::guard('web')->check()) {
            session([
                config('upsoftware.tracking.session_key') => [
    ...
    {
        return Str::limit(Request::cookie(config('upsoftware.tracking.device_cookie'), 'device_uuid'), 255, '');
    }
    ...
    public function setCookieID($id)
    {
        Cookie::queue(
            Cookie::forever(
                config('upsoftware.tracking.device_cookie'),
                $id,
                null,
                null,
                null,
                config('upsoftware.tracking.cookie_http_only'), // http only
                false,
                null // same site
            )
        );
    ...
    public function getHijackingDetector()
    {
        if (!isset($this->hijackingDetector)) {
            $tmp = App::make(config('upsoftware.tracking.hijacking_detector'));
